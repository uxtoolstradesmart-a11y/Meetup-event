...
| Motion permission | Android: works right away. iOS 13+: `DeviceOrientationEvent.requestPermission()` needs a tap, so the page shows an "Enable tilt effect" chip and also asks on a tap of the ticket (must be `click`, not `pointerdown`, or iOS rejects it). If permission is refused, the chip hides and the idle sway stays |
